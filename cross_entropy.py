...
def 0_norm_of_features(controller, tol):
	norm = 0
...
def 1_norm_of_features(controller):
	norm = 0
	for k in sorted(controller.keys()):
		norm+=numpy.absolute(controller[k])
	return norm

def 2_norm_of_features(controller):
	norm = 0
	for k in sorted(controller.keys()):
...
def norm(controller, norm_type)
	if norm_type==0:
		return 0_norm_of_features(controller,0.001)
	elif norm_type==1:
		return 1_norm_of_features(controller)
	elif norm_type==2:
		return 2_norm_of_features(controller)
	else:
...
if __name__ == '__main__':
	...
	parser.add_argument('-no','--norm',action = "store", dest = "norm_type", default = 0, help = "The type of penalty norm to use. Choices are 0, length of feature list, 1 absolute value of features, 2 sum of squared features.")
    ...
    for x in range (0, depth):
        ...
        for a in range(0, controllers):
            ...
            norm_of_weights = norm(random_controller)
	...
	norm_of_weights = norm(start_controller)
        ...
        for g in range(0,test_games):
            ...
            for r in range(0, test_reps):
                ...
                test_res['norm_of_weights'] = norm_of_weights #to allow the norm of the weights of the controller to be seen as output
